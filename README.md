...
为了掌握权值或者决策树，我们利用学习算法的预测来代替每个最初例子的属性。
经常能够准确预测类别的学习算法会取得较高的权值，而不准确的那些则任意被忽略。

为了给既定训练例子获取学习算法的预测，我们首先必须将其运用到原始训练集“排除该样本”中，然后利用最终的分类器，否则委员会就有被拟合学习算法控制的风险，
因为它们可以通过记忆类别来预测准确的类别。
这种类型的元学习被称为“堆叠”

“装袋”算法通过重新取消的方法来产生训练集的随机变量，将同样的学习算法应用到每个训练集中，然后通过投票将结果结合起来。

最聪明的元学习算法之一就是推进

推进算法的名字源于这样的想法：该过程可以推进只比随机猜测好一点的分类器，但如果持续如此，就会接近完美。
...
如果我们切换到马尔科夫网络，可以利用任何逻辑公式作为马尔科夫网络特征的模型，

这样就可以把逻辑和图解模型统一起来了

## 从休谟到你的家用机器人

除了数据，更拥有其他的输入东西：你的原始知识，依据一组逻辑公式，有还是没有权值

## 行星尺度机器学习

如果你不能在可用时间和记忆内完成一件事，那么知道这件事怎么做并没有多大用处

一方面，因为学习和概率推理，简单的ＭＬＮ可以做复杂程序能做的事。

另一方面，当它不起作用时，调试起来就困难很多。解决方法就是让它更加交互，有能力进行反省，并解释它的推理

## 医生马上来看你

治愈癌症的方法是一种能够输入癌症基因组，并输出杀死癌细胞药物的程序

# 第十章 建立在机器学习之上的世界

新的世界正在形成，而机器学习是解锁这个世界的钥匙

## 性、谎言和机器学习

对于所有关心的问题，更好的办法在于集中于特殊、非比寻常、预测能够成功配对的属性上

## 数码镜子

你的终极算法模型甚至可以帮你成为更好的人

## 充满模型的社会

未来的网络空间会是一个巨大的平行世界，

只会选择最有希望的东西在真实世界中进行试验，
...
人们在人际关系、自我实现、灵性中寻找意义，就和现在他们做的一样。

谋生的需要将会变成遥远的记忆，这是我们克服的又一个人类的原始过去。

## 战争不属于人类

如果机器人装备竞赛能加速《日内瓦第五公约》禁止人类参与战争的进程，也许这也是一件好事

## 谷歌 ＋　终极算法　＝　天网？

1. 最大的忧虑是，和所有技术一样，人工智能可能会落入不法之徒手里
...
自然进化最后不会只有两种物种，一种物种顺从另一种物种。

这就意味着下一个瓶颈就是界限，虽然现在我们还没看到它。

# 后记

1. 开始思考

2. 隐私、数据分享、工作的未来、机器人战争、人工智能的承诺与危险之类
